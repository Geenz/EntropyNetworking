...
#include <functional>
 ...
 enum class HttpMethod {
     GET, HEAD, POST, PUT, DELETE_, OPTIONS, PATCH, PROPFIND, MKCOL, MOVE, COPY
 };
 
 // Optional HTTP version preference for a request
 enum class HttpVersionPref { Default, PreferH3, H3Only };
 
 // Optional minimum TLS version
 enum class TlsMinVersion { Default, TLSv1_2, TLSv1_3 };
 ...
 struct RequestOptions {
     ...
     bool followRedirects = false; // honored for aggregated and streaming requests; aggregated defaults to follow for safe methods, streaming GET follows by default
     ...
     std::optional<uint64_t> contentLength; // required for HTTP/2 and preferred; when unset, caller may use chunked (not yet implemented here)
